...
#define ALL_MMUIDX_BITS ((1ULL << NB_MMU_MODES) - 1)
...
static void tlb_flush_by_mmuidx_async_work(CPUState *cpu, run_on_cpu_data data)
{
    ...
    if (to_clean == ALL_MMUIDX_BITS) {
        ...
        qatomic_set(&cpu->neg.tlb.c.part_flush_count,
                    cpu->neg.tlb.c.part_flush_count + ctpop32(to_clean));
        if (to_clean != asked) {
...
void tlb_flush_page_by_mmuidx(CPUState *cpu, vaddr addr, uint32_t idxmap)
{
    tlb_debug("addr: %016" VADDR_PRIx " mmu_idx:%" PRIx32 "\n", addr, idxmap);
...
typedef struct {
    ...
    vaddr len;
    uint32_t idxmap;
    uint32_t bits; // does this really need to be uint32_t or can it stay uint16_t?
} TLBFlushRangeData;
